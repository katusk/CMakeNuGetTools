...
function(_nuget_merge_n_nuspec_files FILEPATH_ACC)
    ...
    set(FILE_ELEMENT_BEGIN_STR "<file src=")
    string(FIND "${LINES_BASE}" "${FILE_ELEMENT_BEGIN_STR}" FIRST_FILE_ELEMENT_BEGIN_POS)
    if(${FIRST_FILE_ELEMENT_BEGIN_POS} EQUAL -1)
        message(FATAL_ERROR "Cannot merge: did not find the \"${FILE_ELEMENT_BEGIN_STR}\" part of the .nuspec files node in \"${FILEPATH_BASE}\".")
    endif()
    # Initialize content of FILEPATH_ACC with FILEPATH_BASE adding comment referring to original source file
    string(SUBSTRING "${LINES_BASE}" 0 ${FIRST_FILE_ELEMENT_BEGIN_POS} NEW_LINES_BASE)
    string(APPEND NEW_LINES_BASE "<!-- Below initialized from: \"${FILEPATH_BASE}\" -->")
    math(EXPR LINES_BASE_AFTER_FILES_NODE_BEGIN_POS "${LINES_BASE_FILES_NODE_BEGIN_POS} + ${FILES_NODE_BEGIN_LEN}")
    string(SUBSTRING "${LINES_BASE}" ${LINES_BASE_AFTER_FILES_NODE_BEGIN_POS} -1 LINES_BASE_AFTER_FILE_NODE_BEGIN)
    string(APPEND NEW_LINES_BASE "${LINES_BASE_AFTER_FILE_NODE_BEGIN}")
